...
def averaged_perceptron(data, labels, params={}, hook=None):
    ...
    theta = np.swapaxes(np.array([ths/(n*T)]),0,1) 
    theta_0 = np.array([th0s/(n*T)]) 
...
def eval_learning_alg(learner, data_gen, n_train, n_test, it):
    ...
    for i in range(it):
        ...
        test_data, test_labels = data_gen(n_train)


        classifier_performance.append(eval_classifier(learner, train_data, train_labels, test_data, test_labels))

    return(sum(classifier_performance)/it)

def eval_learning_alg_test(learner, data_gen, n_train, n_test, it):
    '''
    Test on train data instead of generating new data
    '''    
    ...
    for i in range(it):
        ...
        classifier_performance.append(eval_classifier(learner, train_data, train_labels, train_data, train_labels))
...
def xval_learning_alg(learner, data, labels, k):
    ...
    split = [to_fix.T for to_fix in np.array_split(data.T,k)]
    labels_split = [to_fix.T for to_fix in np.array_split(labels.T, k)]

    scores = []

    for iter in range(k):
        #use slicing to make a copy and not copy by reference
        D_minus_j = split[:]
        D_minus_j_labels = labels_split[:]
        #you didn't need to pop here - could have just used slicing. But this works too.
        D_j = D_minus_j.pop(iter)
        D_j_labels = D_minus_j_labels.pop(iter)

        score_j = eval_classifier(learner, np.concatenate(D_minus_j, axis = 1), np.concatenate(D_minus_j_labels, axis = 1),D_j, D_j_labels)
...
print("perceptron, .1 flip percent probability:")
print(eval_learning_alg(perceptron, gen_flipped_lin_separable(pflip=.1), 20, 20, 5))

print("averaged perceptron, .1 percent probability:")
print(eval_learning_alg(averaged_perceptron, gen_flipped_lin_separable(pflip=.1), 20, 20, 5))

print("perceptron, .25 percent probability:")
print(eval_learning_alg(perceptron, gen_flipped_lin_separable(pflip=.25), 20, 20, 5))

print("averaged perceptron, .25 percent probability:")
print(eval_learning_alg(averaged_perceptron, gen_flipped_lin_separable(pflip=.25), 20, 20, 5))

print("perceptron, .1 flip percent probability, test data:")
print(eval_learning_alg_test(perceptron, gen_flipped_lin_separable(pflip=.1), 20, 20, 5))

print("averaged perceptron, .1 percent probability, test data:")
print(eval_learning_alg_test(averaged_perceptron, gen_flipped_lin_separable(pflip=.1), 20, 20, 5))

print("perceptron, .25 percent probability, test data:")
print(eval_learning_alg_test(perceptron, gen_flipped_lin_separable(pflip=.25), 20, 20, 5))
