...
def test_linear_classifier_with_features(dataFun, learner, feature_fun,
    ...
    if draw:
        ...
        hook = None
    th, th0 = learner(data, labels, hook = hook, params = {'T':1000})
    if hook: hook((th, th0))
    print("Final score", int(score(data, labels, th, th0)))
    print("Params", np.transpose(th), th0)

    return int(score(data, labels, th, th0))
...
def test_with_features(dataFun, order = 2, draw=True, pause=True):
    score = test_linear_classifier_with_features(
        dataFun,                        # data
        perceptron,                     # learner
...
#3a - Enter a list of 6 integers
#indicating the number of polynomial features of degrees [1, 10, 20, 30, 40, 50] for a 2-dimensional feature vector.

feature = np.array([[1,1]]).reshape(-1,1)

degrees = [1, 10, 20, 30, 40, 50] 
transforms = {str(degree):make_polynomial_feature_fun(degree) for degree in degrees}

print("number of terms for degrees [1, 10, 20, 30, 40, 50]:")
print([len(transforms[str(degree)](feature)) for degree in degrees])

#   Example for part 3B) test_with_features()
# print("super_simple_separable data")
# test_with_features(super_simple_separable, 1, draw=False, pause=False)


def get_lowest_order(dataFunc):

    # simple function that returns the lowest possible order for separability for a given dataset
    # does it in a brute force way, starting at order one. The separator should have a score equal to
    # the length of the labels vector for a given dataset, and the loop stops when that is accomplished.

    separated = False

    data, labels = dataFunc()
    max_score = labels.shape[1]
    order = 1

    while separated == False:
        score = test_with_features(dataFunc, order, draw = False, pause = False)
        if score == max_score:
            separated = True
        else:
            order += 1
...
#3b, get lowest order for each dataset

data_funcs = [super_simple_separable_through_origin, super_simple_separable, xor, xor_more]

print("polynomial basis orders for separability for given datasets:")
print([get_lowest_order(data_func) for data_func in data_funcs])

#test_with_features(xor_more, 2, draw=False, pause=False)
