...
def eval_classifier(learner, data_train, labels_train, data_test, labels_test, T = 100):
    th, th0 = learner(data_train, labels_train)#, params = {'T' : T})
    return score(data_test, labels_test, th, th0)/data_test.shape[1]

def xval_learning_alg(learner, data, labels, k, T = 100):
    _, n = data.shape
    idx = list(range(n))
    np.random.seed(0)
...
def load_mnist_data(labels):
    ...
    for label in labels:
        images = load_mnist_single("/Volumes/GoogleDrive/My Drive/MOOCs and Books/6.036/code_and_data_for_hw3/mnist/mnist_train{}.png".format(label))
